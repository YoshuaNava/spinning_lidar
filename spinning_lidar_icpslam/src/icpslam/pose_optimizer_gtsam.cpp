...
void PoseOptimizerGTSAM::addNewKeyframeVertex(PointCloud::Ptr *new_cloud_ptr, Pose6DOF icp_transform, Pose6DOF pose, uint *key)
{
...
/* Inspired on mrsmap by Jurg Stuckler */
void PoseOptimizerGTSAM::addNewOdometryVertex(PointCloud::Ptr *new_cloud_ptr, Pose6DOF pose, uint *key)
{
    *key = curr_vertex_key_;
    graph_poses_.insert(std::pair<uint, Pose6DOF>(*key, pose));
...
void PoseOptimizerGTSAM::addNewEdge(Eigen::MatrixXd cov, uint vertex1_key, uint vertex2_key, uint *key)
{
    *key = curr_edge_key_;
